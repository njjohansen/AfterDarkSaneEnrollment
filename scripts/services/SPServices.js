app.factory('SPServices', [function(){

	// Because of the nature of Sharepoint 2007 lists, same-domain policy issues etc., we need a mocked version of the SPServices object
	// for when we're developing. This factory will take care of all of this, returning the mocked one in development environment, and 
	// automatically switching it to the Sharepoint suited one, when in production.
	// This should ease development of these sort of things significantly
		    ...
		    $.fn.SPServices.SPGetCurrentUser = function(options) {
		        ...
		        $.ajax({
		            ...
		            complete: function (xData, Status) {
		                ...
		                if (opt.fieldName === "ID"){
		                    var elementToParse = $(xData.responseText).find("a[id$='EditItem_LinkText']"),
		                        strToParse = elementToParse.attr('href');


		                    return jQuery.trim(strToParse.split('?ID=')[1].split('&')[0]);
		                };
		...
		var wrappedService = function(options){
			jQuery.fn.SPServices(options);
		};

		// we need to have some sort of representation of this method, because it's being called - but we don't need it to do anything in development
		wrappedService.SPGetCurrentUser = function(){
			return jQuery.fn.SPServices.SPGetCurrentUser();
		};
